...
async function main() {
  ...
  for (const makeId of Object.keys(makes)) {
    ...
    const models = make.modelCodes;
    const modelStrings = Object.entries(models)
      .map(([modelId, modelName]) => `${modelName}(${modelId})`)
      .join(",");
    lines.push(`${make.title}-${makeId}:${modelStrings}`);
  }

  fs.writeFileSync("data/makes-and-models-raw.txt", lines.join("\n"));
  console.log("Scraping completed. Data saved to makes-and-models.txt");
}
